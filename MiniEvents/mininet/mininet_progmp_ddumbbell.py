...
class Minievents(Mininet):
    ...
    def init(self,**kwargs):
        ...
        ### init all hosts
            ...
            sleep(.5)
            self.get('h4').sendCmd('ifconfig h4-eth0 10.0.0.4')
            self.get('h4').waiting = False
            sleep(.5)
            self.get('h5').sendCmd('ifconfig h5-eth0 10.0.0.5')
            self.get('h5').waiting = False
            sleep(.5)
            self.get('h5').sendCmd('ifconfig h5-eth1 11.0.0.5')
            self.get('h5').waiting = False
            sleep(.5)
            self.get('h6').sendCmd('ifconfig h6-eth0 10.0.0.6')
            self.get('h6').waiting = False
    ...
    def iperf(self, **kwargs):
        ...
        kwargs.setdefault('bw', 1000000000)
        if debug:
            info('***iperf event at t={time}: {args}\n'.format(time=time.time(), args=kwargs))
        
        if not os.path.exists("output_10mbits_5ms_1ms_rr"):
            os.makedirs("output_10mbits_5ms_1ms_rr")
        server_output = "output_10mbits_5ms_1ms_rr/iperf-{protocol}-server-{src}-{dst}_{it}_{port}.txt".format(it=iterator,**kwargs)
        client_output = "output_10mbits_5ms_1ms_rr/iperf-{protocol}-client-{src}-{dst}_{it}_{port}.txt".format(it=iterator,**kwargs)
        if debug:
    ...
    def ping(self, **kwargs):
        ...
        src.waiting = False
        if dst.IP() == '10.0.0.5':
            ping_cmd = 'ping -c {count} -i {interval} 11.0.0.5'.format(**kwargs)
            src.sendCmd('{cmd} &>{output}{b} &'.format(cmd = ping_cmd, output=output, b=2))
            src.waiting = False
        if dst.IP() == '10.0.0.2':
            ping_cmd = 'ping -c {count} -i {interval} 11.0.0.2'.format(**kwargs)
            src.sendCmd('{cmd} &>{output}{b} &'.format(cmd = ping_cmd, output=output, b=2))
            src.waiting = False
...
class StaticTopo(Topo):
    ...
    def build(self, number_of_paths = 2, loss = 0):
        ...
        # left side
        self.addLink(h1, s1, bw=10, delay="5ms",max_queue_size=50, loss=float(loss))
        self.addLink(h2, s1, bw=10, delay="5ms",max_queue_size=50, loss=float(loss))
        self.addLink(h2, s2, bw=10, delay="1ms",max_queue_size=50, loss=float(loss))        
        self.addLink(h3, s2, bw=10, delay="1ms",max_queue_size=50, loss=float(loss))
        # middle
        self.addLink(s1, s3, bw=10, delay="5ms",max_queue_size=50, loss=float(loss))
        self.addLink(s2, s4, bw=10, delay="1ms",max_queue_size=50, loss=float(loss))
        # right side
        self.addLink(s3, h4, bw=10, delay="5ms",max_queue_size=50, loss=float(loss))
        self.addLink(s3, h5, bw=10, delay="5ms",max_queue_size=50, loss=float(loss))
        self.addLink(s4, h5, bw=10, delay="1ms",max_queue_size=50, loss=float(loss))
        self.addLink(s4, h6, bw=10, delay="1ms",max_queue_size=50, loss=float(loss))
...
if __name__ == '__main__':
    ...
    parser.add_argument("-ef", "--events_file", help="Events file for minievents", default="./ddumbbell.json")
    parser.add_argument("-num", "--num_runs", help="The number of runs", default=100)
    ...
    for iterator in range(97,int(args.num_runs)):
