...
class Minievents(Mininet):
    ...
    def iperf(self, **kwargs):
        ...
        kwargs.setdefault('port', 5000)
        kwargs.setdefault('bw', 100000)
        info('***iperf event at t={time}: {args}\n'.format(time=time.time(), args=kwargs))
        
        if not os.path.exists("output"):
            os.makedirs("output")
        server_output = "output/iperf-{protocol}-server-{src}-{dst}:{port}.txt".format(**kwargs)
        client_output = "output/iperf-{protocol}-client-{src}-{dst}:{port}.txt".format(**kwargs)
        ...
        if kwargs['protocol'].upper() == 'UDP':
             ...
             iperf_server_cmd = 'iperf3 -s --port {port} -i 1 '.format(**kwargs)
             iperf_client_cmd = 'iperf3 -t {duration} -c {server_ip} -p {port}'.format(server_ip=server.IP(), **kwargs)
       # elif kwargs['protocol'].upper() == 'TCP':
       ...
       #      iperf_client_cmd = 'iperf -t {duration} -c {server_ip}'.format(server_ip=server.IP(), **kwargs)
        ...
        if kwargs['protocol'].lower() == 'tcp':
            while 'Connected' not in client.cmd(
                            'sh -c "echo A | telnet -e A {server_ip} {port}"'.format(server_ip=server.IP(), **kwargs)):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--events",default="minievents2.json", help="json file with event descriptions")
