...
def addRecursive(num):
	...
	elif len(recursiveRegister) == 2:
		recursiveRegister[num] = "R6"
	else:
...
def printNet(out, model):
	...
	# Print switching
	if (model == 'single'):
		#TODO Switching single
		out.write("{}IF (n{} > R2)".format(indent,outnode[0].num))
		out.write(" {\n")
		out.write("{}\tSET(R2, n4);\n".format(indent))
		out.write("{}\tSET(R4, sbf.ID);\n{}".format(indent,indent))
		out.write("}\n")
		out.write("\t}\n")
		out.write("\tVAR swer = SUBFLOWS.FILTER(sbf => sbf.ID == R4);\n")
		out.write("\tFOREACH (VAR s IN swer) {\n")
		out.write("\t\ts.PUSH(Q.POP());\n\t}\n")
	elif (model == 'double'):
