...
private:
  ...
  void SendData ();
public:
  std::vector<int64_t> GetAllRttVar();
  void            SetNeat(NEAT::Network* ntw, NEAT::Network* meta_ntw); // function for setting NEAT NN
  void            DisableMpTcp(); // function for disabling mptcp
  bool            mptcp_enabled = true;  // Bool whether mptcp is enabled
  NEAT::Network*   ntw;            // NEAT Neural Network
  NEAT::Network*   meta_ntw;            // NEAT Neural Network for Meta-Scheduler
  bool            neat_enabled;   // Bool whether Neat is enabled
  std::vector<int64_t> sendLog; // Vector of time_values where packets were send
  std::vector<int64_t> GetTimeLog();
