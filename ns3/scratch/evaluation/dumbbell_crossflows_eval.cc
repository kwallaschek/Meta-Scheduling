...
/** Initialize NEAT-Network**/
  ...
  if(neat){  
    ...
    Config::SetDefault("ns3::MpTcpSocketBase::MaxUnprobedMinRtt", UintegerValue(30)); 
  }
  if(meta){
    Config::SetDefault("ns3::MpTcpBulkSendApplication::Neat", BooleanValue(true));
...
// Set Error model onto sinkapp 
 ...
 //nodeCE.Get (1)->SetAttribute ("ReceiveErrorModel", PointerValue (em));
...
// 
  ...
  //monitor->SerializeToXmlFile ("resultsfafda2.xml" , true, true );
  ...
  cj = (sink_2->GetTotalRx ()*8)/(1000000*(time-2));
  //if(sink_3->GetTotalRx () > 0)
  ah = (sink_3->GetTotalRx ()*8)/(1000000*(time-2));
