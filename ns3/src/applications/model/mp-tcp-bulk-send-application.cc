...
//                    MakeUintegerChecker<uint32_t>(1))
    ...
    .AddAttribute("FlowType",
                  ...
                  MakeStringChecker())
    .AddAttribute("Neat",
                  "Enable Neat NN as Scheduler",
                  BooleanValue(false),
                  MakeBooleanAccessor(&MpTcpBulkSendApplication::neat_enabled),
                  MakeBooleanChecker())
  ...
  if (!m_socket)
    {
      ...
      // Set Neat network if its there
      if (neat_enabled){
        m_socket->SetNeatNN(ntw, meta_ntw);
      }
      // Disable MpTcp if wanted
      if (!mptcp_enabled){
        m_socket->DisableMpTcp();
      }
    ...
    { // Time to send more new data into MPTCP socket buffer
      ...
      if (m_maxBytes > 0)
      ...
      else
        {
          ...
          // Add this timestamp to the log vector
          //std::cout << "Pushed back: " << Simulator::Now().GetMilliSeconds() << std::endl;
          sendLog.push_back(Simulator::Now().GetNanoSeconds());
    }
...
void MpTcpBulkSendApplication::SetNeat (NEAT::Network* ntw, NEAT::Network* meta_ntw)
{
  NS_LOG_FUNCTION(this);
  this->ntw = ntw;
...
void MpTcpBulkSendApplication::DisableMpTcp ()
{
  NS_LOG_FUNCTION(this);
  this->mptcp_enabled = false;
}

std::vector<int64_t> 
MpTcpBulkSendApplication::GetTimeLog()
{
  NS_LOG_FUNCTION(this);
  return this->sendLog;
}
std::vector<int64_t>
MpTcpBulkSendApplication::GetAllRttVar()
{
  return m_socket->GetAllRttVar();
}
